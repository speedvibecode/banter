    ...
    <section className={`section-panel grid ${compact ? "gap-3 p-4" : "gap-4 p-5"}`}>
      <div className={compact ? "grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3" : "flex items-start justify-between gap-4"}>
        <div>
          ...
          <Link
            href="/reputation"
            className={`mt-2 inline-block font-[var(--font-space)] font-bold uppercase tracking-[-0.05em] text-[color:var(--text)] transition hover:text-[color:var(--primary)] ${compact ? "text-[1.35rem]" : "text-3xl"}`}
          >
            {progression.title}
          </Link>
        </div>
